...
(adv) Unlike a full-fledged CPU, there's no program counter. Usually CPUs read their own instructions, so they need a register to store the next instruction to read. In my case though, instructions come in over USB.
...
\begin{frame}{Results}
	\begin{itemize}[<+->]
		...
		\item $26\times$ speedup compared to software \only<5->{(maybe?)}
